...
class Db{
    ...
    # 判断是否为连接丢失异常.
    static private function isbreak(string $str, int $i) : bool {
        Log::write("数据库异常: ({$i}) {$str}", 'ERROR');
        ...
        if($this->_trans_level == 1){
            ...
            do{
                try{
                    ...
                    if($this->isbreak($e->errorInfo[2] ?? '', $times)){
                        $this->putConn(null);
                        if(0 == $times){
                            throw $e;
                        }
        ...
        if($this->_trans_level > 0){
            ...
            if(0 == $this->_trans_level){
                try{
                    ...
                    if($this->isbreak($e->errorInfo[2] ?? '', 0)){
                        $this->putConn(null);
                    }else{
                        $this->putConn($this->conn);
                    }
    ...
    public function execute(string $sql, array $params, int $operation, int $flags = \PDO::FETCH_ASSOC) : ?int {
        ...
        do{
            try{
                ...
                //TODO: 连接断开判断.
                if(static::isbreak($e->errorInfo[2] ?? '', $times)){
                    $this->putConn(null);
